...
def rewire_layers(ls, perm, seed=None):
    ...
    import os as _os
    _trials = int(_os.environ.get("QMILL_SABRE_TRIALS", "4"))  # was 10000; bond is defect-bounded (~8) so routing quality barely matters, and trials=10000 spawns ~28 procs and dominates runtime (~50s/cycle)
    _heur = _os.environ.get("QMILL_SABRE_HEUR", "decay")        # 'basic' routes MUCH faster than 'decay' (the rewire bottleneck on big-qubit circuits); quality matters less since bond is defect-bounded
    ss = SabreSwap(coupling_map=CouplingMap.from_line(ls[0].num_qubits), heuristic=_heur, trials=_trials, seed=seed)
    qc = ss(qc)
