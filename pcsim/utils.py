...
DEVICE = "cuda:0" if torch.cuda.is_available() else "cpu"   # CPU fallback for no-CUDA boxes (M4 Max)
def to_backend_cuda(x, device=DEVICE):
    # On CPU use numpy: its LAPACK SVD is far more robust than torch-CPU's
    # (which raises LinAlgError "failed to converge" on the ill-conditioned
    # matrices the unswap sweeps produce). torch path kept for real CUDA GPUs.
    if device == "cpu":
